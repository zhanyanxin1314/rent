...
class HousepController extends BaseController
{

    public function actionIndex()
    {
	$list = Housep::find()->orderBy([ 'id' => SORT_DESC ])->all();

	return $this->render("index",[
		'models' => $list
	]); 
    } 
    public function actionCreateHousep()
    {	
		if( \Yii::$app->request->isGet ){
			$id = $this->get("id",0);
			$info = [];
			if( $id ){
				$info = Housep::find()->where([ 'id' => $id ])->one();
			}
			return $this->render("_createhousep",[
				"info" => $info
			]);
		}
		...
		$date_now = time();
		if(empty($title)){
			return $this->renderJSON([],"参数名称不能为空!",-1);
		}
		$housep_info = Housep::find()
			->where([ 'title' => $title ])->andWhere([ '!=','id',$id ])
			->one();
		if( $housep_info ){
			return $this->renderJSON([],"该参数名称已存在，请输入其他的参数名称",-1);
		}
		...
		if( $info ) {
			...
			$house_model = new Housep();
			$house_model->created_at = $date_now;
		}
		$house_model->title = $title;
		$house_model->cat = $cat;
		$house_model->status = 10;
		$house_model->updated_at = $date_now;
